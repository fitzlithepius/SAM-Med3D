...
def random_sample_next_click(prev_mask, gt_mask, method="random"):
    ...
    if method.lower() == "random":
        ...
        fn_mask_single = F.pad(
            fn_masks[None, None], (1, 1, 1, 1, 1, 1), "constant", value=0
        ).to(torch.uint8)[0, 0]
        fp_mask_single = F.pad(
            fp_masks[None, None], (1, 1, 1, 1, 1, 1), "constant", value=0
        ).to(torch.uint8)[0, 0]
        ...
        if len(all_points) == 0:
            ...
            point = torch.tensor(
                [gt_mask.shape[0] // 2, gt_mask.shape[1] // 2, gt_mask.shape[2] // 2]
            )
            is_positive = False
        else:
            point = all_points[np.random.randint(len(all_points))]
...
def sam_model_infer(
    ...
    num_clicks=1,
):  # Added num_clicks for iterative prompting if desired
    """
    ...
    if roi_gt is not None and (roi_gt == 0).all() and num_clicks > 0:
        ...
        return (
            np.zeros_like(roi_image.cpu().numpy().squeeze()),
            None,
        )  # Return None for low_res_mask

    with torch.no_grad():
        input_tensor = roi_image.to(device)
        image_embeddings = image_encoder(input_tensor)
        ...
        if prev_low_res_mask is None:  # Initialize low_res_mask for the decoder
            ...
            data = torch.load("gt_embeds.pt")
            sparse_embeddings = data["sparse_prompt_embeds"].to(device)
            dense_embeddings = data["dense_prompt_embeds"].to(device)
            print(
                f"image_embeds.shape: {image_embeddings.shape}, sparse_embed.shape{sparse_embeddings.shape}"
            )
            image_embedding_check = image_embeddings.cpu().equal(
                data["image_embeds"].cpu()
            )
            assert image_embedding_check is True, "Image Embeddings are not the same"
            low_res_masks, _ = mask_decoder(
                image_embeddings=image_embeddings,
                prompt_embeddings=sparse_embeddings,
                dense_embeddings=dense_embeddings,
                original_sizes=[(1024, 1024, 1024)],
            )
            print(low_res_masks.shape)
            masks_check = torch.allclose(
                low_res_masks.cpu(), data["mask"].cpu().to(torch.bfloat16), 1e-5, 1e-8
            )
            similarity = F.cosine_similarity(
                low_res_masks.flatten(), data["mask"].flatten(), dim=0
            )
            assert similarity > 0.99, f"Predicted masks are not the same: {similarity}"
    ...
    #             align_corners=False,
    #         )
    #         current_prev_mask_for_click_generation = (
    #             torch.sigmoid(current_prev_mask_for_click_generation) > 0.5
    #         )
    #         break
    #     # Final high-resolution mask from the last low_res_masks
    #     final_masks_hr = F.interpolate(
    #         low_res_masks,  # Use the final low_res_masks
    #         size=roi_image.shape[-3:],
    #         mode="trilinear",
    #         align_corners=False,
    #     )
...
def get_roi_from_subject(subject_canonical, meta_info, crop_transform, norm_transform):
    ...
    meta_info["padding_params_functional"] = (
        padding_params  # (d_pad_neg, d_pad_pos, h_pad_neg, h_pad_pos, w_pad_neg, w_pad_pos) from tio
    )
    meta_info["cropping_params_functional"] = (
        cropping_params  # (d_crop_neg, d_crop_pos, ... )
    )
...
def get_subject_and_meta_info(img_path, gt_path):
    ...
    subject = tio.Subject(image=tio.ScalarImage(img_path), label=tio.LabelMap(gt_path))
...
def data_postprocess(roi_pred_numpy, meta_info):
    ...
    pred_label_map_roi_space = tio.LabelMap(
        tensor=roi_pred_tensor, affine=meta_info["roi_subject_affine"]
    )
    ...
    resampler_to_original_grid = tio.Resample(
        target=reference_image_original_space, image_interpolation="nearest"
    )

    # Perform the resampling.
    pred_resampled_to_original_space = resampler_to_original_grid(
        pred_label_map_roi_space
    )

    # Extract the data as a NumPy array.
    # The tensor from TorchIO will be (C, D, H, W). Squeeze the channel dimension.
    final_pred_numpy_dhw = (
        pred_resampled_to_original_space.data.squeeze(0).cpu().numpy()
    )
...
def validate_paired_img_gt(
    ...
    exist_categories, final_pred_numpy_original_grid = get_category_list_and_zero_mask(
        gt_path
    )
    print(f"final prediction: {final_pred_numpy_original_grid.shape}")
    ...
    for category_index in exist_categories:
        print(
            f"category_index: {category_index}, existing categories: {exist_categories}"
        )
        category_specific_subject = copy.deepcopy(subject)
        category_specific_meta_info = copy.deepcopy(meta_info)
        # roi_image is (1,1,D,H,W)
        # ground truth: roi_label is (1,1,D,H,W)
        ...
        print("cls_pred_original_grid: ", cls_pred_original_grid.shape)
